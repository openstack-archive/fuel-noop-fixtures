...
describe Noop::Task do
  ...
  context 'base' do
    ...
    it 'should have inspect' do
      expect(subject.inspect).to eq 'Task[Manifest: my/test.pp Spec: my/test_spec.rb Hiera: novanet-primary-controller.yaml Facts: ubuntu16.yaml Status: pending]'
    end
  end
  ...
  context 'facts' do
    it 'has file_name_facts' do
      ...
      expect(subject.file_name_facts.to_s).to eq 'ubuntu16.yaml'
    end
    ...
    it 'has file_path_facts' do
      ...
      expect(subject.file_path_facts.to_s).to eq "#{root}/facts/ubuntu16.yaml"
    end
